<?php
/**
 * admin/index.php — Essence of Varanasi Tour CRM Dashboard Overview
 */
...
try {
    ...
    echo '<p><strong>Quick Fix:</strong> Click below to run database setup & seed initial tour admin user:</p>';
    echo '<a href="/admin/seed.php?token=tour-seed-2026" style="display: inline-block; background: #dc2626; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">Run Database Setup & Seed Admin User →</a>';
    echo '</div>';
    exit;
}
...
try {
    ...
    $upcomingTravel  = (int)db()->query("SELECT COUNT(*) FROM bookings WHERE check_in >= CURDATE() AND status != 'cancelled'")->fetchColumn();
    $confirmedCount  = (int)db()->query("SELECT COUNT(*) FROM bookings WHERE status = 'confirmed'")->fetchColumn();

    // Recent 10 Tour Inquiries
...
<div class="metrics-grid">
  <div class="metric-card">
    <div class="metric-title">Total Tour Inquiries</div>
  ...
  <div class="metric-card">
    <div class="metric-title">Upcoming Travels</div>
    <div class="metric-value" style="color: #d97706;"><?= number_format($upcomingTravel) ?></div>
  </div>
  <div class="metric-card">
    <div class="metric-title">Confirmed Tours</div>
...
<div class="admin-card">
  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
    <h2 style="font-size: 1.15rem; font-weight: 700;">Recent Tour &amp; Experience Inquiries</h2>
    <a href="/admin/bookings.php" class="btn btn-outline btn-sm">View All Inquiries →</a>
  </div>

  <?php if (!$recent): ?>
    <p style="color: var(--color-muted); padding: 12px 0;">No tour inquiries recorded yet. Customer web form submissions will appear here.</p>
  <?php else: ?>
